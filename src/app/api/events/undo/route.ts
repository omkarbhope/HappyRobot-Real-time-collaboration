...
import { UndoSchema } from "@/features/undo/schemas";
...
export async function POST(request: NextRequest) {
  return withAuth(async (userId) => {
    ...
    const result = await undoService.undo(parsed.data.eventId, userId);
    if (!result) return jsonError("Event not found or cannot undo", 404, "NOT_FOUND");
